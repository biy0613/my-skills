...
### 2.4 FnGuide — 분기 손익계산서·현금흐름표 (2026-08 경로 전면 교체)

> 🔴 **구버전 `comp.fnguide.com/SVO2/ASP/*.asp` 는 2026년 8월 폐쇄됐다.** `SVD_Finance` · `SVD_Corp` · `SVD_Main` 전부 죽었다. **404가 아니라 `200 OK` + "페이지가 없습니다 / 신버전 바로가기" 안내 페이지(약 1,800바이트)를 돌려준다** — status_code만 검사하면 정상으로 오인한다. 신버전은 HTML 스크래핑이 아니라 **JSON API**다.

```
GET https://wcomp.fnguide.com/CompanyInfo/getFinIncome     ← 손익계산서
GET https://wcomp.fnguide.com/CompanyInfo/getFinCashFlow   ← 현금흐름표
    ?cmp_cd={code}&freq_typ=Q&consol_typ=M
Headers: X-Requested-With: XMLHttpRequest
         Referer: https://wcomp.fnguide.com/CompanyInfo/Finance
```

- `freq_typ` — `Q` 분기 / `Y` 연간. **`Y`도 마지막에 최근 분기를 덧붙여 온다**(`2023/12, 2024/12, 2025/12, 2026/03`). 구버전과 같은 함정이므로 아래 간격 검사를 그대로 유지한다.
- `consol_typ` — `M` 연결(구 `pGB=1` 대응) / `I` 별도.

응답 구조:

```json
{"dataset": {
  "header": [{"YYMM":"2025/06","CD":"VAL1"}, ..., {"YYMM":"2026/03 (최근분기)","CD":"VAL4"},
             {"YYMM":"2025/03 (전년동기)","CD":"VAL5"}, {"YYMM":"전년동기대비(%)","CD":"VAL6"}],
  "data":   [{"NAME":"매출액(수익)","LVL":0,"VAL1":"2987.09", ...}, ...]}}
```

- 항목명은 `NAME`(HTML 태그가 섞여 오므로 제거 필요), 값은 `VAL1~VAL6`, `LVL`은 들여쓰기 깊이.
- 손익 주요 행: `매출액(수익)` · `영업이익` · `영업이익(발표기준)` · `당기순이익` · `(지배주주지분)당기순이익`. **`영업이익`이 `영업이익(발표기준)`보다 먼저 나오므로 startswith 매칭이 안전하다.**
- 현금흐름 첫 행: `영업활동으로인한현금흐름`. 헤더에 `(최근분기)` 라벨 없이 순수 날짜 4개만 온다.
- 단위는 **억원** (구버전과 동일).
- **`VAL5`(전년동기 실적값)가 그대로 오므로 흑자전환 판정이 쉬워졌다.** 구버전은 `전년동기(%)`가 전년동기 적자 시 공란이라 별도 재분류가 필요했다. 이제 `VAL5 ≤ 0 and VAL4 > 0`으로 직접 판정할 수 있다.

> ⚠️ **연간 응답은 마지막에 최근 분기를 덧붙인다.** "날짜 컬럼이 모두 `/12`이면 연간"이라는 판별은 **틀린다** — 연간의 월 집합도 `{12, 3}`이라 분기로 오판된다. 실제로 이 오판 때문에 신세계의 4개 분기 누적 영업이익이 **17,946억(3년+1분기)** 으로 잡힌 적이 있다. 3배 넘게 부풀려지는데 에러는 나지 않는다.
>
> 올바른 판별: **연속한 날짜 컬럼의 간격이 전부 3개월인가.** 연간은 `{12,3}`, 분기는 `{3}`으로 확실히 갈린다. `screener.py`의 `is_quarterly()`가 이 방식이며, 신버전에서도 그대로 유지한다.

`screener.py`의 `fetch_fn()` → `_fn_frame()` → `pick_quarter_tables()`가 이 JSON을 **구버전 HTML 테이블과 같은 모양의 DataFrame**으로 변환한다(`항목` 컬럼 + 날짜 컬럼 + `전년동기` + `전년동기(%)`). 껍데기를 맞춰 둔 덕분에 `analyze()`의 판정 로직은 손대지 않았다.
...
### 3.1 FnGuide — 종목코드 파라미터명을 틀리면 조용히 삼성전자를 준다

구버전 `SVD_Main.asp`는 `gicode`와 무관하게 항상 삼성전자를 반환하는 캐시 버그가 있었다. **신버전으로 옮겨오면서 이 함정이 형태를 바꿔 살아남았다.**

신버전의 종목 파라미터는 **`cmp_cd`** 하나뿐이다. `gicode`·`code` 같은 다른 이름을 주거나 아예 빠뜨리면 — 오류도, 빈 응답도 아니고 — **삼성전자(005930) 데이터가 정상 응답으로 반환된다.**

```
/CompanyInfo/Finance?cmp_cd=000990  →  DB하이텍(000990)   ✅
/CompanyInfo/Finance?gicode=A000990 →  삼성전자(005930)   ❌ 조용히 틀림
/CompanyInfo/Finance                →  삼성전자(005930)   ❌ 조용히 틀림
```

전 종목 루프에서 이걸 만나면 **모든 종목의 실적이 삼성전자로 채워진다.** 검증법: 응답의 `<title>종목명(코드) | …` 에서 코드를 읽어 요청한 코드와 대조한다. `verify_sources.py`의 `[함정] FnGuide 파라미터명` 항목이 매 실행 시 이걸 확인한다.

선행 PER은 여전히 §2.5의 `flag=2`로 직접 계산한다 — FnGuide 화면값을 신뢰하지 않는다.
